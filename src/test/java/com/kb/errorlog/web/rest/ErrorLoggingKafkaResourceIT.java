...
import com.kb.errorlog.config.KafkaProperties;
...
class ErrorLoggingKafkaResourceIT {
    ...
    void setup() {
        ...
        ErrorLoggingKafkaResource kafkaResource = new ErrorLoggingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/error-logging-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/error-logging-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
